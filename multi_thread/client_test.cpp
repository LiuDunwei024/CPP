 ...
 * @LastEditTime: 2024-06-09 22:27:27
...
int init_socket(std::string ip, std::string port) {
        if ( -1 == client.Socket()) {
                return -1;
        }
...
void *recv_message(void *arg) {
        while (1) {
                while (0 != client.Recv()) {
                        if (memcmp(client.recv_buff, "Exit", sizeof("Exit"))|| 
                            memcmp(client.recv_buff,"exit", sizeof("exit"))) {
                                return NULL;
                            }
